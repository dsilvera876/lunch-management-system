-- ============================================================
-- Late-order menu snapshot integrity
-- ============================================================
--
-- Invariant: a late order may use ONLY a lunch-day/menu snapshot that was
-- frozen during the actual order day. Historical snapshots must never be
-- reconstructed from the current recurring menu.
--
-- Future scheduler contract (not implemented here):
-- A background worker/systemd timer should run shortly after midnight
-- Jamaica time each business day and call
--   public.materialize_provider_snapshots_for_order_date(current_order_date)
-- for the current Jamaica order date. That freezes every active provider
-- menu before Staff normal ordering begins, so zero-order providers still
-- have an immutable snapshot available for next-morning HR late ordering.
--
-- Normal Staff ordering may still lazily call private.ensure_provider_lunch_day
-- for the current order day only.
...
revoke all on function private.jamaica_today_date() from public, anon, authenticated;

create or replace function private.lookup_provider_lunch_day_snapshot(
  p_provider_id uuid,
  p_order_date date
)
returns uuid
...
begin
  ...
  where ld.provider_id = p_provider_id
    ...
    and ld.order_date = p_order_date;

  return v_lunch_day_id;
end;
$$;

revoke all on function private.lookup_provider_lunch_day_snapshot(uuid, date)
